...
from graph import Graph
# from generate import GraphGenerator # Забрано імпорт, щоб уникнути циклічної залежності
import sys

# Використовуємо велике число для представлення "нескінченності"
INF = sys.maxsize
...
class Algorithm:
    def __init__(self, g: Graph):
        self.g = g
        # Ця матриця буде зберігати найкоротші шляхи між усіма вершинами
        # Її обчислення ініціюється під час створення об'єкта
        self.full_distances_matrix = self._floyd_warshall()
        ...
        for i in range(V):
            for j in range(V):
                if i == j:
                    ...
                    dist[i][j] = INF  # Встановлюємо INF там, де немає прямого ребра

        # Основні цикли алгоритму
        for k in range(V):
            for i in range(V):
                for j in range(V):
                    # Перевіряємо наявність шляхів перед додаванням, щоб уникнути переповнення INF + INF
                    if dist[i][k] != INF and dist[k][j] != INF:
                        if dist[i][j] > dist[i][k] + dist[k][j]:
                            dist[i][j] = dist[i][k] + dist[k][j]
    ...
    def find_tsp_nearest_neighbor(self, start_city=0):
        """
        Евристика найближчого сусіда для TSP, використовуючи матрицю повних відстаней.
        Повертає маршрут (список вершин) та загальну довжину.
        """
        n = self.g.n
        visited = [False] * n
        route = []
        ...
        for _ in range(1, n):
            ...
            if nearest_city != -1:
                ...
                # Це вказує на проблему зв'язності, яка мала бути виявлена раніше
                return None, INF
    ...
    def execute_tsp(self):
        ...
        # Перевіряємо зв'язність графа перед запуском TSP
        if any(self.full_distances_matrix[i][j] == INF for i in range(self.g.n) for j in range(self.g.n)):
            print("Помилка: Граф не є повністю зв'язним.")
            print("Неможливо виконати TSP, що охоплює всі вершини.")
            return None, INF
...
# --- Приклад використання (додано для демонстрації) ---
if __name__ == "__main__":
    from generate import GraphGenerator  # Локальний імпорт для прикладу

    # Створюємо об'єкт графу з параметрами
    # Щільність 0.8 забезпечує високу ймовірність зв'язності/гамільтоновості
    graph = Graph(n=10, d=0.8)
